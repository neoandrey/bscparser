...
namespace bsc_parser{
  ...
  public ConnectionProperty(string source, string sourceDatabase){
      try{
		if (!File.Exists(AppDomain.CurrentDomain.BaseDirectory+"\\..\\db\\storekeeper.sqlite"))
		{
			SQLiteConnection.CreateFile(AppDomain.CurrentDomain.BaseDirectory+"\\..\\db\\storekeeper.sqlite");
		}
		liteConnect = new SQLiteConnection("Data Source="+AppDomain.CurrentDomain.BaseDirectory+"\\..\\db\\storekeeper.sqlite;Version=3;");
		liteConnect.Open();	
